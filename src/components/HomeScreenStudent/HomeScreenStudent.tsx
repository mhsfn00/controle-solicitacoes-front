...
import { BookUser, FileCheck2, UserCog, LogOut, Check, User, ArrowLeft } from "lucide-react";
...
// Componente Principal da Tela (agora como default export)
    ...
    return (
        <div className="flex bg-gray-50 min-h-screen">
            ...
            <main className="flex-1 p-8 ml-[280px] mr-[280px]">
                ...
                <section>
                    <h2 className="text-xl font-semibold">Situação de Exame de Qualificação</h2>
                                ...
                                {qualificationSteps.map((step, index) => (
                                    <div key={index} className="flex gap-4 mb-4 last:mb-0">
                                        <div className="flex flex-col items-center">
                                            <span className={`flex items-center justify-center w-8 h-8 rounded-full ${index === 0 ? 'bg-black text-white' : 'bg-gray-200 text-gray-600'}`}>
                                        ...
                                        <div className={`pt-1 ${index === 0 ? 'font-bold' : ''}`}>
                                            <p className="text-sm">{step.date} - {step.description}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
